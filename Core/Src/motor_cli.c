 ...
 *   ADC1_2    1   SpeedControl_Task -- the control loop, paced by TIM6 TRGO
 *   LPUART1   3   this CLI
 *
 * LPUART1 sits strictly below the control loop, so a character arriving
 * mid-tick waits for the tick to finish; the tick never waits. The ISR itself
...
static void PrintStatusLine(void)
{
  ...
  if (SpeedControl_IsLoopStalled())
  {
    BufStr(&b, "  loop=STALLED");
  }
  BufStr(&b, CLI_EOL);
...
/* Called from LPUART1_IRQHandler in stm32g4xx_it.c. The vector itself stays in
 * the CubeMX-generated file so regenerating the project does not produce a
 * duplicate symbol. */
